...
class WriteExcel(object):
    ...
    def write_non_conformity(self,last_rows,jiraname,jiraid,nonconformityitem, responsible, checktime):  # 检查到不符合项，调用这个函数写到表格里
        path = Filepath()
        summarypath = path.summary_report()
        xls = xlrd.open_workbook(summarypath, formatting_info=True)
        newxls = copy(xls)
        ...
        if cell1==0 and cell2==0 and cell3==0 and cell4==0:
            ...
            project_sheet.write(last_rows - 1, 1, nonconformityitem)
            project_sheet.write(last_rows - 1, 2, responsible)
            project_sheet.write(last_rows - 1, 3, checktime)
        else:
            project_sheet.write(last_rows, 0, jiraid)
            project_sheet.write(last_rows, 1, nonconformityitem)
            project_sheet.write(last_rows, 2, responsible)
            project_sheet.write(last_rows, 3, checktime)
        newxls.save(summarypath)